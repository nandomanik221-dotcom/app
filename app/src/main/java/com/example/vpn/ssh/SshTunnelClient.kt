...
class SshTunnelClient(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                VpnLogManager.log(
                    LogLevel.CONN,
                    "SSH",
                    "[SSH] Menghubungkan ke $serverHost:$serverPort, batas waktu 15 detik"
                )
                ...
                /*
                 * Trilead itself will create the socket used for the
                 * HTTPProxyData connection. We provide a local bridge
                 * which exposes our already-prepared transport stream
                 * as a fake local HTTP proxy.
                 */
                val server =
                    ServerSocket(
                        0,
                ...
                val job = bridgeScope.launch {
                    try {
                        ...
                        /*
                         * ServerSocket.soTimeout only affects accept().
                         * Explicitly configure the accepted socket too.
                         */
                        ...
                        /*
                         * Trilead HTTPProxyData sends:
                         *
                         * CONNECT host:port HTTP/1.0/1.1
                         *
                         * Validate it instead of blindly returning 200.
                         */
                        ...
                        /*
                         * Drain HTTP proxy request headers.
                         */
                        while (true) {
                        ...
                        /*
                         * This bridge is only a local adapter.
                         * The real proxy/TLS/payload negotiation has already
                 ...
                 *
                 * The hostname/port here are the logical SSH destination.
                ...
                /*
                 * Give the bridge enough time to accept the local Trilead
                 * connection before entering SSH KEX.
                 */
                try {
                    bridgeReady.await()
                } catch (e: Exception) {
                    throw IllegalStateException(
                        "SSH local transport bridge failed",
                        e
                    )
                }
                    ...
                    connection.connect(
                        object : ServerHostKeyVerifier {
                            override fun verifyServerHostKey(
                                ...
                                /*
                                 * Host-key pinning is not available in the
                                 * current VpnProfile model, so preserve the
                                 * existing compatibility behaviour.
                                 *
                                 * The SSH cryptographic exchange itself still
                 ...
                 * IMPORTANT:
                 *
                 * Use Trilead's own authenticated connection ping.
                 * This verifies that the actual SSH connection is alive
                ...
                /*
                 * Only mark the backend running after:
                 *
                 * TCP
                 * -> proxy
                 * -> optional TLS
                ...
                /*
                 * Verify at least one real direct-tcpip channel.
                 *
                 * This is separate from SSH ping:
                 * SSH ping proves the authenticated SSH transport is alive;
                 * direct-tcpip proves the server allows forwarding.
                 */
    ...
    override fun createTunnelSocket(
        ...
        if (!isRunning.get() || !isTunnelReady.get()) {
            throw IllegalStateException(
     ...
     *
     * IMPORTANT:
     * The TCP port itself never decides whether TLS is used.
     *
    ...
    private fun establishTransportSocket(
        ...
        /*
         * MUST happen before socket.connect().
         */
        val protected =
...
ALPN=$alpn"""
        ...
        if (hasRemoteProxy) {
            ...
            /*
             * Remote proxy TCP connection.
             *
             * No special handling based on port number.
             */
            ...
            when (proxyType) {
                "SOCKS5" -> {
                    ...
                    if (isSsl) {
                        ...
                        VpnLogManager.log(
                            LogLevel.CONN,
                            "TLS",
                            "[TLS] TLS_CONNECTING (Target: $cleanSni:$targetPort)"
                        )
                ...
                "HTTPS" -> {
                     ...
                     * TLS is terminated at the proxy first.
                     */
                    currentStage = "TLS"
                    ...
                    /*
                     * Only establish target TLS when explicitly enabled.
                     */
                    if (isSsl) {
                        currentStage = "TLS"
                    ...
                    if (isSsl) {
                        ...
                        VpnLogManager.log(
                            LogLevel.CONN,
                            "TLS",
                            "[TLS] TLS_CONNECTING (Tunneled Target: $cleanSni:$targetPort)"
                        )
    ...
    private fun performHttpProxyConnect(
        ...
        val response =
            HttpStatusParser.consumeSingleResponse(input)
                ?: throw IllegalStateException(
    ...
    private fun performSocks5ProxyHandshake(
        ...
        if (version != 0x05) {
            ...
            if (
                ...
                throw IllegalStateException(
                    "Remote Proxy returned HTTP text instead of SOCKS5. " +
                        "Set Remote Proxy Type to HTTP."
                )
            }
        ...
        when {
            ...
            method == 0xFF -> {
                throw IllegalStateException(
                    "SOCKS5 Proxy authentication method rejected"
                )
            }
...
sec_key=PRESENT"""
        ...
        if (
            ...
            if (response != null) {
                ...
                /*
                 * Handle informational/intermediate HTTP responses.
                 */
                while (
                    peekIsHttp(
                        pushback,
    ...
    /**
     * Verify the authenticated SSH connection and direct-tcpip forwarding.
     *
     * This function is STRICT.
     *
     * If every forwarding probe fails, it throws instead of merely logging
     * a warning. This prevents false SSH_READY state.
     */
    private fun verifyEndToEndConnectivity(
        connection: Connection
    ) {
        /*
         * First verify the authenticated SSH transport.
         */
        try {
            connection.ping()
         ...
         *
         * Opening a LocalStreamForwarder is already a meaningful test:
         * it requires the SSH server to accept channel forwarding and
         * establish the requested outbound connection.
         */
        val probeTargets =
            listOf(
                "1.1.1.1" to 80,
        ...
        for ((host, port) in probeTargets) {
            ...
            try {
                ...
                /*
                 * Successfully opening the direct-tcpip channel is the
                 * important part. Optionally send a tiny HTTP request for
                 * HTTP ports, but do not require a response body.
                 */
                val output =
                    forwarder.outputStream

                val input =
                    forwarder.inputStream
                ...
                if (port == 80) {
                    ...
                    /*
                     * Do not block forever waiting for a remote HTTP response.
                     * The successful SSH direct-tcpip channel itself is enough
                     * to establish forwarding capability.
                     */
                    try {
                        forwarder
                            .inputStream
     ...
     *
     * WebSocket-specific header manipulation occurs ONLY when
